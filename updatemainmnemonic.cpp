...
void UpdateMainMnemonic::on_comboBox_type_of_unit_currentTextChanged(const QString &arg1)
{
    ...
    list_unit_name.clear();
    ui->comboBox_unit->clear();
    list_unit_name = data_base_action->get_unique_values_with_condition("unit_name",
                                                                        "units",
                                                                        condition);
...
void UpdateMainMnemonic::input_unit()
{
    ...
    if (ui->comboBox_unit->findText(unit_name) == -1) {
        ...
        unit_id = data_base_action->id_exist_data("unit_id", "units", condition);
    }
    data_base_action->database_refresh();
}
void UpdateMainMnemonic::on_pushButton_INPUT_clicked()
{
    ...
    input_unit();
    updating_data();
    data_base_action->update_main_mnemonic(main_mnemonic_id,
                                           main_mnemonic_name,
...
void UpdateMainMnemonic::search_current_data()
{
    ...
    main_mnemonic_description = data_base_action->find_with_condition("main_mnemonic_description",
                                                                      "main_mnemonics",
                                                                      condition);
