...
void UpdateToolWindow::search_current_data()
{
    ...
    description = data_base_action->find_with_condition("description",
                                                        "tool_descriptions",
                                                        condition);
    length = data_base_action->find_with_condition("length", "tool_descriptions", condition);

    outer_diameter = data_base_action->find_with_condition("outer_diameter",
                                                           "tool_descriptions",
                                                           condition);
    inner_diameter = data_base_action->find_with_condition("inner_diameter",
                                                           "tool_descriptions",
                                                           condition);
...
void UpdateToolWindow::on_pushButton_INPUT_clicked()
{
    input_produser();
    updating_data();
    data_base_action->update_tool(tool_id,
                                  tool_name,
                                  tool_description_id,
                                  description,
                                  length,
