...
void UpdateSensor::on_pushButton_INPUT_clicked()
{
    input_method();
    updating_data();
    data_base_action->update_sensor(sensor_id,sensor_name,tool_id,method_id,sensor_description_id,sensor_description,offset);
...
void UpdateSensor::search_current_data()
{
    ...
    sensor_description_id=data_base_action->id_exist_data("sensor_description_id","sensors",condition);
    ...
    sensor_description=data_base_action->find_with_condition("sensor_description","sensor_descriptions",condition);
